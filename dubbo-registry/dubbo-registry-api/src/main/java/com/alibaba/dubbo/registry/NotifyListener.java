...
public interface NotifyListener {
     ...
     * 3. 中途变更时，允许不同类型的数据【分开】通知，比如：providers, consumers, routers, overrides，允许只通知其中一种类型，
     *          但该类型的数据必须是【全量】的，不是增量的。<br>
     * 4. 如果一种类型的数据【为空】，需通知一个empty协议并带category参数的标识性URL数据。<br>
     * 5. 通知者(即注册中心实现)需保证通知的【顺序】，比如：单线程推送，队列串行化，带版本对比。<br>
     *
