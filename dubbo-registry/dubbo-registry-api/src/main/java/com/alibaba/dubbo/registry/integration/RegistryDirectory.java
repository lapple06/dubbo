 ...
 * RegistryDirectory
 *
 *  RegistryDirectory 实现了 NotifyListener 接口，当注册中心节点信息发生变化后，RegistryDirectory
...
public class RegistryDirectory<T> extends AbstractDirectory<T> implements NotifyListener {
     ...
     *
     * registryDirectory的关键所在。这里会写methodInvokerMap。在doList中会读这个集合；同时会销毁无用的服务
     * 1.将监听到的url转换成<url , Invoker> 集合
     * 2.进一步将<url , Invoker> 转换成 <methodName , List<Invoker>>
     * 3.将结果值合并到methodInvokerMap中
     *
     *  注意，注册中心会通知全量有效的url集合, 这里会把不在集合里的url销毁掉
     *
    ...
    private void refreshInvoker(List<URL> invokerUrls) {
        ...
        if (invokerUrls != null && invokerUrls.size() == 1 && invokerUrls.get(0) != null
            ...
            try {
                // 销毁无用的 Invoker，避免服务消费者调用已下线的服务的服务。
    ...
    /**
     * doList可以看做是对methodInvokerMap的读操作
     * 写操作发生在继承了NotifyListener的notify方法中
     */
    @Override
