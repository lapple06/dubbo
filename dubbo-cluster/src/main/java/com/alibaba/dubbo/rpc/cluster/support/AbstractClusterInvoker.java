...
public abstract class AbstractClusterInvoker<T> implements Invoker<T> {
     ...
     *
     *
     *  invokers参数可以看作是活着的invoker
     *  selected可以看作是最近重试时调用失败的invoker
     *
     *  sticky粘滞特性：开启了粘滞特性表示让服务消费者尽量调用同一个服务提供者，调用同一个服务提供者，除非该提供者挂了再进行切换
     *                  但是并不保证一定会调用同一个服务提供者，当上次粘滞的提供者挂掉或者上次调用失败重试，就不会再粘滞
     *
     */
    protected Invoker<T> select(LoadBalance loadbalance, Invocation invocation, List<Invoker<T>> invokers, List<Invoker<T>> selected) throws RpcException {
        ...
        boolean sticky = invokers.get(0).getUrl().getMethodParameter(methodName, Constants.CLUSTER_STICKY_KEY, Constants.DEFAULT_CLUSTER_STICKY);
        {
            ...
            // 这里可以把invokers入参当作是活着的invoker的列表
     ...
     *
     *  1.通过负载均衡选出invoker
     *  2.若选出的invoker不可用或不稳定，则reselect
     *  3.若reselect出来的invoker为null, 则使用invoker的下一个
     *
     */
